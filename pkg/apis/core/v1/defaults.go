...
func defaultPodRequests(obj *v1.Pod) {
	...
	for key, aggrCtrLim := range aggrCtrReqs {
		if _, exists := podReqs[key]; !exists && resourcehelper.IsSupportedPodLevelResource(key) && !resource.IsHugePageResourceName(key) {
...
func defaultPodLimits(obj *v1.Pod) {
	...
	for key, aggrCtrLim := range aggrCtrLims {
		if _, exists := podLims[key]; !exists && resourcehelper.IsSupportedPodLevelResource(key) && resource.IsHugePageResourceName(key) {
